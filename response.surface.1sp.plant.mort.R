...
bingo <- list()
fecundity.stats <- matrix(NA, nrow=length(N0s), ncol=5)
colnames(fecundity.stats) <- c(
	"starting.plants",
	"ending.plants",
	"biomass",
	"total.fecundity",
...
for(i in seq_along(N0s)){
	N0 <- N0s[i]
	x0 <- c(50,N0,N0*params_middle$b)
	growing.season <- fecundity_dynamics(x0, T2, T2/100.)

	biomass <- growing.season[nrow(growing.season),4]
	plants <- growing.season[nrow(growing.season),3]
	seeds <- growing.season[nrow(growing.season),4]*conversion
	fecundity <- seeds/plants
...
# dev.new()
setEPS()
postscript('./response_surface_1sp_plantmort.eps', width=13.5, height=3.25)
# par(mfrow=c(2,2), 
par(oma=c(0,0,0,0), mar=c(4.5,5,1,1))
...
legend(
	0.50,
	100,
	c("50 plants", "10 plants", "1 plant"),
	box.lty=0,
	lty=3:1
)

plot(
	fecundity.stats[,"ending.plants"],
	fecundity.stats[,"total.fecundity"],
	type='l',
	col=1,
	lty=1,
	xlab="Seed-producing plants",
...
plot(
	fecundity.stats[,"ending.plants"],
	fecundity.stats[,"per.capita.fecundity"],
	type='l',
	col=1,
	lty=1,
	xlab="Seed-producing plants",
...
plot(SPtotal[,1], SPtotal[,2], type='l', col=1, lty=2, xlab="", ylab="Viable seeds in the seed bank", ylim=c(0,100))
plot(SPtotal[,1], SPtotal[,3], type='l', col=1, lty=2, xlab="Years", ylab="Seed-producing plants")
plot(
	x=SPtotal[,1],
	y=SPtotal[,4],
